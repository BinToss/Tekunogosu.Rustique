...
pub const RUSTIQUE_USER_AGENT: &str = concat!(
    env!("CARGO_PKG_NAME"),
    "/",
    env!("CARGO_PKG_VERSION"),
    "  (github: Tekunogosu/Rustique)"
);
...
impl ApiClient {
    ...
    pub async fn fetch_mod(&self, mod_id: impl StrRef) -> Result<Mod, RustiqueError> {
        ...
        if mod_id.is_empty() {
            ...
            return Err(RustiqueError::MalformedModInfoJson(
    ...
    pub async fn fetch_mods_parallel(
        &self,
        mod_list: Vec<ModID>,
    ) -> Result<HashMap<ModID, Mod>, RustiqueError> {
        ...
        let mut tasks: Vec<JoinHandle<Result<(ModID, Mod), String>>> =
    ...
    pub async fn fetch_game_versions(&self) -> Result<HashSet<String>, RustiqueError> {
        ...
        let versions: GameVersions =
            serde_json::from_str(&text).map_err(|e| RustiqueError::SimpleError(e.to_string()))?;
